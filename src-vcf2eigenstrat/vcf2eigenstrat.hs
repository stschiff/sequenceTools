...
import SeqTools.Fasta (readFastaEntry)
...
main :: IO ()
main = do
    withFile "/data/schiffels/ReferenceGenomes/hs37d5.fa" ReadMode $ \fh -> do
        let prod = fromHandle fh
        (chrom, seqProd) <- readFastaEntry
        err $ chrom
...
--                             Just fp -> Just <$> (liftIO $ loadRefSeq chrom fp)
...
--                               Just seq_ -> let nuc = seq_ `B.index` (snpPos - 1)
--                                            in  if nuc == snpRef
